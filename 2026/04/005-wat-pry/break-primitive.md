# `(:wat::pry::break)` — the binding.pry shape
...
This is the load-bearing primitive. Everything else in the pry
...
2. **Captures the CALL_STACK** — arc 016 already populates a
   thread-local Vec<FrameInfo> on every function entry/exit. Pry
   reads it directly.
3. **Hands control to the pry loop** — reads from the program's
   stdin (which, under `wat --pry`, is the user's terminal via
...
4. **Resumes on `:continue`** — the loop returns from
   `pry::break` with `:()`. The function continues at the next
...
The pry-shaped UX comes from a defmacro shipped in
`wat/std/pry.wat`:
...
Now `(:wat::pry::break)` works wherever `stdin`/`stdout`/`stderr`
...
wat-pry (broken @ trade.wat:42:7) compute-decision>
...
wat-pry (broken)> (:trading::action regime rsi vol)
:Action::Buy

wat-pry (broken)> (:wat::pry::frames)
...
wat-pry (broken @ trade.wat:120) :user::main>

wat-pry (broken)> :down
[walked back]
wat-pry (broken @ trade.wat:42:7) compute-decision>

wat-pry (broken)> :continue
...
rest of the body produces. Pry prompt reappears at the top level.

## The capture mechanism — what the substrate must do

`eval_form` for `:wat::pry::break-with-stdio` in
`src/runtime.rs` does:

```rust
fn eval_pry_break_with_stdio(
    args: &[Value],
    env: &Environment,
    world: &FrozenWorld,
    ...
    // Run the inline pry loop
    pry_loop(world, &captured_env, &captured_frames, stdin, stdout, stderr)?;

    Ok(Value::unit())
}
```

`pry_loop` reads lines, parses, evals against `captured_env`,
writes results, recognizes `:continue` / `:exit` / `:up` / etc.
The loop returns when the user types `:continue`; then
`eval_pry_break_with_stdio` returns `Value::unit()` and the
...
a pry against it. But Ruby is mutable — while the user inspects,
...
Wat's `(:wat::pry::break)` captures an Environment in a frozen
world. While the user inspects:

- The Environment is an Arc-cloned snapshot. The original keeps
  evolving in the calling thread (it isn't, because pry is
...
The freeze invariant turns pry into a static-time inspection
running over runtime values.

This is the user's point: **rust being frozen is a blessing,
not a curse**. Ruby has to fight its own mutability to make pry
...
that frame's call site). `:up` switches the pry loop's eval
...
moves the pry loop's eval scope.
...
- An inline loop that reads, parses, evals, writes — uses
  ...
  `:wat::eval-edn!`.
- Recognition of pry-command keywords (`:continue`, `:up`,
  etc.) at the wat level, in `wat/std/pry.wat`.

Estimated ~300 lines of Rust + ~150 lines of wat. The pry loop
(`:wat::pry::serve`) is shared between bare-mode and break-mode;
