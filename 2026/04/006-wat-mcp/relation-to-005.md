...
|---|---|
| `:wat::pry::break-with-stdio` | The break primitive itself. 006 wraps it in an MCP-aware variant; doesn't reimplement. |
...
| `FrameInfo::env` extension | 005 slice 2 adds Environment to FrameInfo for `:up`/`:down` walking. 006 reuses for `:wat::pry::eval-in-frame`. |
| `:wat::pry::ls` | The agent calls this through `wat-eval` to discover the SymbolTable. |
| `:wat::pry::show` | The agent calls this through `wat-eval` to read source. |
| `:wat::pry::completions` | Could be exposed for agent-side completion if it helps; not strictly needed since agent has full string composition. |
| `:wat::pry::where` / `:wat::pry::frames` | Used during break-mode for agent-side stack inspection. |
| `:wat::pry::last-error` | Same — agent reads via `wat-eval`. |
| `wat/std/pry.wat` (the loop) | Pattern reference for `wat/std/mcp.wat`'s implementation. |

Roughly: **005 builds the substrate's introspection +
break-and-resume mechanics. 006 wraps them in JSON-RPC.**

Without 005's slice 2, there's no `(:wat::pry::break)` for an
agent to hit. Without 005's slice 1, there's no `:wat::pry::ls`
...
| `:wat::pry::override-return` | Useful for agents doing counterfactual reasoning; humans rarely want this (humans usually want `:continue` and let the function complete). |
| `:wat::pry::eval-in-frame` (with explicit frame index) | Humans use `:up` / `:down` to walk; agents may prefer one-shot frame addressing without state. |
| `:wat::mcp::dispatch` and the JSON-RPC method routing | Wat code in `wat/std/mcp.wat`; matches `:wat::pry::dispatch` shape. |
| `:wat::mcp::main` (cli entry-point for MCP mode) | Symmetric to `:wat::pry::main`. |
...
|---|---|
| 006-1 (basic MCP eval) | 005-1 (`:wat::pry::ls` + `:wat::pry::show`) + wat-json arc |
| 006-2 (break-as-notification) | 005-2 (`:wat::pry::break-with-stdio` + FrameInfo::env) |
| 006-3 (override-return + eval-in-frame primitives) | 006-2 |
| 006-4 (TCP attach for remote MCP) | Either 005-5 (TCP attach for human pry) or 006-3 |

So the natural order:

1. 005 slice 1 (bare pry mode + introspection)
...
Where `TYPE` is `pry` for 005 and `mcp` for 006. Same shape;
...
stdio. Both reuse the same underlying pry primitives.
...
- Cons: 005's scope is "pry-shape REPL for humans"; 006's
...
"one scratch per architectural concern." Pry is the concern;
...
Considered: extract the introspection accessors (`:wat::pry::ls`
/ `:wat::pry::show` / `:wat::pry::completions`) into a separate
battery that both wat-pry and wat-mcp depend on.

This would:
- Pros: clear that introspection is general-purpose; the battery
  could be loaded by users who want neither pry nor MCP but
  do want runtime symbol lookup.
- Cons: more workspace crates; the introspection is tightly
  coupled to pry semantics (Environment capture for break).
  Splitting them might be premature.

**Lean: defer.** Ship wat-pry with introspection inside it. If
a non-pry caller surfaces (e.g., a battery that wants to do
...
- 005-wat-pry — pry-mode entry-point + read-eval-print over stdio
...
1. Open arc XXX-wat-pry (slice 1 ships).
2. Open arc XXX+1-wat-pry-break (slice 2 ships).
