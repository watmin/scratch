# Substrate primitives — the `:wat::pause::*` namespace
...
when the cli passes `--pause` (see `gating.md`).

## Rust-side primitives (in `crates/wat-pause/`)

These are the `#[wat_dispatch]` shims the pause battery contributes.
Each lives in `crates/wat-pause/src/lib.rs` (or split across modules
as the surface grows).

### `:wat::pause::main`

```
:wat::pause::main (stdin  :wat::io::IOReader)
                (stdout :wat::io::IOWriter)
                (stderr :wat::io::IOWriter)
                -> :()
```

The cli's pause-mode entry-point. Implementation: prints a banner,
calls `:wat::pause::serve` in a loop, handles top-level command
dispatch (e.g., `:reload` → exit non-zero so the cli's parent
re-forks).

Could be a wat-level define (in `wat/std/pause.wat`) instead of a
Rust shim. The choice is whether we want the entry-point in
substrate Rust or substrate wat. Wat-level is more honest (pause
...
```
:wat::pause::serve (in  :wat::io::IOReader)
                 (out :wat::io::IOWriter)
                 (err :wat::io::IOWriter)
                 -> :()
```
...
`wat/std/pause.wat`**, not Rust — it's a tail-recursive wat
function that:

1. Writes prompt to `out`.
2. Reads one line from `in`.
3. Recognizes pause commands (`:ls`, `:show`, `:exit`, etc.) and
   dispatches.
...
Bare-mode: invoked by `:wat::pause::main` against the program's
stdio.

Break-mode: invoked by `:wat::pause::break-with-stdio` against the
...
calls `:wat::pause::serve` with the captured scope as eval context,
...
### `:wat::pause::completions`

```
:wat::pause::completions (prefix :String) -> :Vec<:String>
```
...
```rust
fn eval_pause_completions(args: &[Value], world: &FrozenWorld) -> Result<Value, RuntimeError> {
...
```
:wat::pause::ls (prefix :Option<String>) -> :Vec<:Symbol>
```

Same walk as `completions` but returns `:Symbol` values (which
print as keyword-paths) for the user to read directly. Used by
the `:ls` pause command. Optional prefix narrows results.

### `:wat::pause::show`

```
:wat::pause::show (sym :Symbol) -> :Option<:String>
```
...
```
:wat::pause::where () -> :Option<:wat::kernel::Location>
```
...
```
:wat::pause::frames () -> :Vec<:wat::kernel::Frame>
```
...
### `:wat::pause::last-error`

```
:wat::pause::last-error () -> :Option<:wat::kernel::Failure>
```
...
values. Pause just exposes a reader. Substrate addition: a
`OnceLock<Option<Failure>>` populated by the panic hook,
read-only from this primitive.

## Wat-side definitions (in `wat/std/pause.wat`)

The pause battery's `wat_sources()` returns this file. Loaded
...
(:wat::core::define
  (:wat::pause::serve
    ...
    -> :())
  (:wat::core::let*
    (((_ :()) (:wat::io::IOWriter/print out "wat-pause> "))
     ((line-opt :Option<String>) (:wat::io::IOReader/read-line in)))
    (:wat::core::match line-opt -> :()
      (:None ())                                    ;; EOF — exit cleanly
      ((Some line)
        (:wat::core::cond -> :()
          ((:wat::pause::is-command? line) (:wat::pause::dispatch-command line in out err))
          (else                          (:wat::pause::eval-and-print line out err)))
        (:wat::pause::serve in out err)))))           ;; tail call
```

Each command handler (`:wat::pause::dispatch-command`,
`:wat::pause::eval-and-print`, `:wat::pause::is-command?`) is its own
wat define, also in `wat/std/pause.wat`.

### Command dispatchers

Wat-level defines for each pause command:

```scheme
(:wat::core::define (:wat::pause/cmd-ls   (args :String) (out :IOWriter) -> :()) ...)
(:wat::core::define (:wat::pause/cmd-show (args :String) (out :IOWriter) -> :()) ...)
(:wat::core::define (:wat::pause/cmd-where (args :String) (out :IOWriter) -> :()) ...)
;; etc.
```
...
```scheme
(:wat::core::cond
  ((:wat::core::string::starts-with? line ":ls")    (:wat::pause/cmd-ls    rest out))
  ((:wat::core::string::starts-with? line ":show")  (:wat::pause/cmd-show  rest out))
  ((:wat::core::string::starts-with? line ":exit")  (:wat::pause/cmd-exit))
...
- **Plugin manifest.** Batteries that want to add pause commands
...
- **History storage.** `~/.wat_pause_history` file. Frontend.

## What the cli needs

The cli's role is small:

1. Parse `--pause` from argv.
2. If set, prepend the pause battery to the battery list.
3. If pause mode, look up `:wat::pause::main` instead of
...
- **Substrate Rust:** ~600 lines across `crates/wat-pause/src/`
  (8 primitives, the panic-hook reader, the FrameInfo extension).
- **Substrate wat:** ~250 lines in `wat/std/pause.wat` (loop,
...
Roughly 1,400 lines for a pause surface with `binding.pry`-style
...
EDN+newline, polymorphic eval, and frozen-world semantics, pause
