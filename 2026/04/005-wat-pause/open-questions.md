...
- **Vocabulary:** the word is *pause*, not REPL. Flag is `--pause`.
  Namespace `:wat::pause::*`. Entry-point `:wat::pause::main`.
- **Two modes, one freeze pipeline:** bare (`wat --pause`) vs
  entry-loaded (`wat --pause <entry>`). Same code path; differ
  only in whether an entry source is loaded.
- **Gating:** pause battery registered only when `--pause` set.
  Without the flag, freeze fails on any pause reference.
- **No `cd`:** wat is FQDN everywhere. Tab completion is the
  navigator.
- **The break primitive captures Environment + CALL_STACK; runs
  the pause loop with the captured scope; resumes on `:continue`.**
- **The pause loop is wat code, not Rust.** Lives in
  `wat/std/pause.wat`. Extensible by batteries.
- **`:user::main` callable from the prompt.** It's just a
  symbol in the frozen world.

## Open — substrate-level

### 1. `:wat::pause::main` — wat-shipped or Rust shim?

**Lean:** wat-shipped (in `wat/std/pause.wat`).

The cli's pause-mode entry-point. Could be either:

- **Rust shim:** `crates/wat-pause/src/lib.rs::pause_main` does the
  banner-print + signal-handling + delegation to `serve`.
- **Wat-level:** `wat/std/pause.wat::wat::pause::main` does the same
  in wat code, calling `:wat::io::IOWriter/println` for the
  banner and `:wat::pause::serve` for the loop.

Wat-level is more honest (pause is "just a wat program"). Lean
...
- **Wat-level:** A wat function `:wat::pause::pretty-print` that
  walks an EDN string and re-formats. Slower but no Rust
  changes.

**Lean:** start wat-level for pause's needs (slice 1); promote to
...
Pause's `show-doc` reads doc-strings attached to functions /
...
- `:wat::pause::show` extension: print the doc above the
  signature.
...
- Multi-tenant: backend spawns a child pause session per
...
- Disciplined: pause break acquires a process-wide "pause lock";
...
Pause break just reads/writes the same way any other code does.
...
The base prompt is `wat-pause>`. In break-mode it becomes
`wat-pause (broken @ file:line:col) function-name>`. Should
...
### 9. History file location — `$HOME/.wat_pause_history` vs `$XDG_STATE_HOME/wat-pause/history`

XDG basedir spec is the right answer; `$HOME/.wat_pause_history`
is the convenient one. Both are common.

**Lean:** XDG, with `$HOME/.wat_pause_history` as fallback if
XDG vars aren't set.
...
When the user types `:reload` in pause mode, two interpretations:
...
### 11. Should bare-pause have a way to load source after start?

`wat --pause` opens with no entry. Should the user be able to
...
If we DO want this, the cleanest shape is "bare-pause remembers a
...
### 12. Programs that conditionally include pause forms

User code that says "only call `(:wat::pause::break)` if some
debug flag is set" — is that supported?

The strict gating answer: no. Pause forms are illegal at freeze if
`--pause` isn't set; conditional code paths don't help because
...
calls `(:wat::pause::break)` or no-ops. But the conditional form
itself can only freeze in `--pause` mode.

**Lean:** keep the strict gate. Document the workaround.

**Resolves:** locked. The trade is that pause is a developer-time
tool, not a runtime feature.

### 13. Pause forms in third-party batteries' wat sources

A battery's `wat_sources()` could in principle include pause
forms. If the battery is registered in non-pause mode, freeze
fails on the pause references.

**Lean:** batteries are responsible for not shipping pause forms
in their core sources. If a battery wants pause-aware features,
it ships them in a separate wat file that's only loaded when
the pause battery is also loaded (the user opts in by linking
both).

**Resolves:** convention only. Document in the battery-author
guide.

### 14. Test code that uses pause — how does it register?

`wat::test!` macros run programs in sandboxed in-process freezes.
If a test wants to exercise pause primitives:
...
The test opts in explicitly. The pause battery is just another
battery; tests register it the same way the cli does.

**Lean:** documented in the test-harness section of pause's
USER-GUIDE entry.
