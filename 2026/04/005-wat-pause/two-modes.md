...
[Quote preserved as spoken; arc originally named `wat-pry`,
renamed `wat-pause` on 2026-05-03.]
...
wat --pause <entry.wat>    → freeze entry → invoke :wat::pause::main
wat --pause                → freeze with no entry → invoke :wat::pause::main
```

Same freeze pipeline runs in all three cases. The pipeline:

1. Argv parse, decide pause mode (`--pause` flag) and entry path
   (if given).
2. Build battery list — pause battery prepended if `--pause` set.
3. Run `startup_from_source`:
   - Config setters from entry source fire (if entry given).
   - All baked battery sources load (`wat/std/*.wat`,
     `wat/std/pause.wat` if pause-mode).
...
For `wat --pause trade.wat`:

- `(:wat::config::set-capacity-mode! :error)` at trade.wat top-level
  → fires during config pass. Pause session inherits this config.
- `(:wat::config::set-dim-router! ...)` → fires. Pause session uses
...
- `(:wat::core::struct :trading::Candle ...)` → registered. Pause
...
- No user defines. The frozen world has only what the batteries
  contributed.
- `:wat::pause::main` is invoked.

Bare pause is a calculator-shaped session against the substrate's
...
This runs the entry program from inside the pause session, using
the same stdio handles pause is using. Output appears in the user's
terminal; stdin reads come from the user's terminal. When main
returns, the pause prompt reappears.

This is the pause "step into the entry point" workflow, falling out
...
```
$ wat --pause trade.wat
wat-pause> (:trading::types::Candle/new 50000.0 50500.0 49500.0 50250.0 1.5)
#trading.types.Candle {:open 50000.0 :high 50500.0 :low 49500.0 :close 50250.0 :volume 1.5}

wat-pause> (:trading::rsi candle)        ;; experiment with a function
0.6234

wat-pause> (:user::main stdin stdout stderr)   ;; run the actual program
[program runs to completion]

wat-pause> (:wat::pause::last-error)       ;; inspect what happened
:None
...
- `wat --pause trade.wat` opens a pause session against the trader's
  symbols.
...
declare a main can be loaded into a pause session via the entry
...
```
$ wat --pause lib.wat
wat-pause> (:my::add 2 3)
5
```

The cli looks up `:wat::pause::main` (not `:user::main`); it doesn't
care whether the entry declared `:user::main`. So pause-mode works
on library files. Useful for "open this file's symbols and let me
play with them."

For normal `wat lib.wat` (no `--pause`), the cli would fail to find
...
The cli's only role is orchestration. It doesn't know what `pause`
is structurally — it just knows "in pause mode, look up
`:wat::pause::main` instead of `:user::main`." The substrate's
...
*the only thing pause mode does at the cli level is load the pause
